...
The durable software architecture (DDD + Clean Architecture + SOLID adapted to Cognite Flows) lives in **`docs/architecture/`**. Read it before creating new modules, layers, or services. This document (`CLAUDE.md`) defines **how** to write each piece; `docs/architecture/` defines **where** code lives and **how dependencies flow**.
